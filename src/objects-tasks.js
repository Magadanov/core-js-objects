...
function sellTickets(queue) {
  ...
  for (let i = 1; i < queue.length; i += 1) {
    if (queue[i] !== 25 && queue[i] - 25 > change) return false;
    change += 25;
  }
  return true;
}
...
class Selector {
  ...
  pseudoElement(value) {
    this.occur.pseudoElement += 1;
    this.checkOrder(6, 'pseudoElement');
    this.result += `::${value}`;
    return this;
  }
  ...
  checkOrder(order, occur) {
    if (this.occur[occur] > 1) {
      throw new Error(
